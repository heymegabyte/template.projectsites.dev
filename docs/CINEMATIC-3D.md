...
1. **Pointer + scroll parallax depth** — ✅ **SHIPPED (2026-09-15, `2099f75`).** A `uParallax` uniform offsets `uv` so the whole field shifts with the visitor; a passive pointermove+scroll target is RAF-lerped (smooth, never costs INP), wired ONLY on the motion-allowed path (reduced-motion → static, no listeners). Default-ON for all 14 industry scenes. Verified: shader gate all-12-uModes non-black + 22 backdrop tests (4 new parallax-wiring) + build + bundle contains `uParallax`.
