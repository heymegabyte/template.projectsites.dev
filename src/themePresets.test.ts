...
const RADIUS_KEYS: (keyof ThemePreset["radius"])[] = [
  "sm",
  "md",
  "lg",
  "xl",
...
const DURATION_KEYS: (keyof ThemePreset["motion"]["duration"])[] = [
  "fast",
  "base",
  "slow",
  "scroll",
...
describe("themePresets", () => {
  it("every preset has a structurally complete bundle", () => {
    for (const name of PRESET_NAMES) {
      ...
      for (const k of RADIUS_KEYS)
        expect(typeof p.radius[k], `${name}.radius.${k}`).toBe("string");
      for (const k of SHADOW_KEYS)
        expect(typeof p.shadow[k], `${name}.shadow.${k}`).toBe("string");
      expect(p.motion.easing, `${name}.motion.easing`).toContain(
        "cubic-bezier",
      );
      for (const k of DURATION_KEYS)
        expect(p.motion.duration[k], `${name}.motion.${k}`).toMatch(/ms$/);
    }
  });
  ...
  it("classic is the default and reproduces the historical look verbatim", () => {
    ...
    expect(c.font).toEqual({
      heading: "Space Grotesk",
      body: "Inter",
      mono: "JetBrains Mono",
    });
    expect(c.radius).toEqual({
      sm: "0.375rem",
      md: "0.75rem",
      lg: "1rem",
      xl: "1.5rem",
    ...
    expect(c.motion.duration).toEqual({
      fast: "150ms",
      base: "250ms",
      slow: "450ms",
      scroll: "1200ms",
  ...
  it("resolvePreset never throws and falls back to classic on bad input", () => {
    ...
    for (const bad of [undefined, null, "", "nope", 42, {}, []]) {
      expect(resolvePreset(bad as unknown), String(bad)).toBe(
  ...
  it("covers the full businessClass enum from brandSchema", () => {
    ...
    for (const cls of ENUM)
      expect(Object.keys(CLASS_TO_PRESET), cls).toContain(cls);
  });
  ...
  it("every non-classic preset has a :root[data-style] flourish in index.css", () => {
    ...
    for (const name of PRESET_NAMES) {
      ...
      expect(
        css,
        `${name} needs a :root[data-style="${name}"] flourish`,
      ).toContain(`:root[data-style="${name}"]`);
    }
  });
});
