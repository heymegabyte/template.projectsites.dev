 ...
 * variant's STRUCTURAL contract jsdom can assert: default-ON (killswitch VITE_SCROLL_STACK=0);
 * `stack` flips the `<ol>` from the
...
const steps = [
  {
    ...
    description: "Find your listing in seconds.",
  },
  { title: "We build it", description: "AI assembles a gorgeous site." },
  { title: "You go live", description: "Hosted, SSL, live in minutes." },
];

describe("scrollStackEnabled — VITE_SCROLL_STACK gate (default-ON, killswitch)", () => {
  it("is ON by default (promoted) and OFF only when VITE_SCROLL_STACK=0", () => {
    expect(scrollStackEnabled()).toBe(true);
    vi.stubEnv("VITE_SCROLL_STACK", "0");
...
describe("ProcessSteps — stack (deck) vs grid variant", () => {
  it("default is the horizontal GRID — no scroll-stack class, connector rail present", () => {
    const { container } = render(
      <ProcessSteps steps={steps} headline="How we do it" />,
    );
  ...
  it("stack flips to the VERTICAL deck — .ps-scroll-stack, no grid, connector rail dropped", () => {
    const { container } = render(
      <ProcessSteps steps={steps} headline="How we do it" stack />,
    );
    const ol = container.querySelector("ol.process-steps") as HTMLElement;
    ...
    expect((items[1] as HTMLElement).style.getPropertyValue("--step-i")).toBe(
      "1",
    );
