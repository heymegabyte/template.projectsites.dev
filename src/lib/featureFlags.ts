 ...
 * dropped; <2 real words → renders nothing). ON by default (AL-832, promoted experimental→default
 * after cinematicProcess/particleField — safe BY CONSTRUCTION + vision-verified gorgeous); a build
 * OPTS OUT with `VITE_KINETIC_MARQUEE=0`. When ON, `Home` renders {@link KineticMarquee} as a band
 * between the services and collection sections; when OFF a shipped site is byte-for-byte unchanged.
 *
 * Safe BY CONSTRUCTION — pure native CSS (NO library, NO JS scroll listener — verified: zero
 * useEffect/addEventListener; unlike GSAP/Lenis): the continuous drift is a universally-supported
 * keyframe; the scroll coupling lives inside `@supports (animation-timeline: scroll())` +
 * `@media (prefers-reduced-motion: no-preference)`. LCP-safe (a BELOW-fold `aria-hidden` decorative
 * band, never the hero LCP element; drift starts at identity, scroll nudge is ±4%), INP-safe
 * (`animation-timeline: scroll(root)` is compositor-driven, off the main thread), CLS-safe
 * (transform-only inside `overflow:hidden`). Firefox keeps the drift without scroll coupling;
 * `prefers-reduced-motion: reduce` → the ribbon is perfectly STATIC + fully legible, so the
 * default-on flip can never dark/jank a section. Source: Framer velocity-marquee + Awwwards SOTD.
 */
export function kineticMarqueeEnabled(): boolean {
  return import.meta.env.VITE_KINETIC_MARQUEE !== "0";
}
